...
from rubik import RubikProblem
import rubik.cube as cube
import numpy as np
from rubik.IDA_star import heuristic1, get_corner_string
...
def dict_state_move(c, moves):
    rs = dict()
    val = -1
    cc = cube.Cube(c)
    for move in moves:
        getattr(cc,move)()
        st = np.array(cc.get_color_list())
        corner = get_corner_string(st)
        # if heuristic1(corner) == 11:
        rs[corner] = val
        val -= 1
    return rs
